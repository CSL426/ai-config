...
- 安裝改成版本目錄加一條連結:`~/.local/bin/ai-config` 指向
  `~/.local/share/ai-config/versions/<版號>/`。更新是換連結,不再覆寫正在執行的
  檔案 —— 那在 Windows 本來就做不到,所以更新得交給背景 PowerShell 並在檔案還沒
  換掉時就回 0。舊的安裝在下次更新時自動收進版本目錄。保留最近五版,
  `acg versions` 看有哪些,`acg update <版號>` 切回去(已在磁碟上就只換連結,不下載)。
- 指令跑完會提示有新版,並就地問要不要更新,答應就直接裝。提示只讀快取、
  不打網路,非互動時完全跳過,`--force` 不會代你回答。
  (`maybe_notify_update` 先前寫好了但沒有任何地方呼叫。)
- `install.sh` 取版號時若 `grep` 沒有命中,`set -e` 會讓整個安裝中止。

- 排程的 `memory push` 不再因為設定目錄有未提交的改動而整個取消。記憶是自己
  一個範圍,`claude/` 底下還沒決定要不要提交的東西與它無關;先前那個取消讓
  一整晚的排程沒跑,而印出來的建議正是剛剛失敗的那道指令。手動選單一工具時
  仍然照擋 —— 那是人在選,漏掉一塊值得停下來說。
- 規則區塊補上工作線交接的用法。`=== HANDOFF ===` 是 remember 用來說日誌檔位置的,
  和 `acg memory handoff` 的工作線是兩回事,新會話看到只知道有個檔案要寫,不知道
  該先 list、該問使用者、該在收工前 write。規則是每個會話必讀的地方,skill 不是。
...
- `memory handoff list [專案路徑]` 可以讀別的專案的交接線；`/acg:pickup` 第一個
  引數是目錄時照此列出，且只有一條可接時直接認領，不再多問一次。
- `pull` 會先清掉只有換行差異、沒有實質內容的修改（Windows `core.autocrlf` 下
  任何重寫都會留下這種標記，包括 acg 自己的 init），不再讓它們擋住 fast-forward。
  遠端與本機同時改到同一檔案時，建議改成走得通的 commit 或 stash，不再叫人先 push——
  落後遠端時 push 一定拒絕，那是個死結。
- `memory autopush enable` 拿到超出 0 到 23 的小時會先拒絕，再也不會把這台在共用
  時間表裡的時段寫成讀不回來的值。測試也改成一律使用暫時的時間表——先前有測試
  直接改寫了這台真正的時段（04:10 變回 04:00），並隨下一次 push 送了出去。

## 1.0.57 - 2026-09-15

- 外掛指令不再誤執行範例文字。`/acg:handoff` 原本會照著說明裡的佔位符建立一則名字
  就叫「名稱」的交接,因為 `!` 開頭的行會被直接執行。改成由模型自己組指令。
- 外掛版號跟著專案走,並加測試比對。它從 1.0.45 一路落後到 1.0.56 都沒人發現,
  因為發版流程完全不檢查那個檔案。
- 新增外掛指令檔的內容檢查:不得含會誤執行的佔位符,每個指令都要有 description。

## 1.0.56 - 2026-09-15

- `autopush enable` 不帶參數時真的會照時間表分配。先前它傳的是預設小時而不是「未指定」,
  所以每次都走手動指定那條路,讓位邏輯從來沒被執行過,兩台永遠都停在同一分鐘。

## 1.0.55 - 2026-09-15

- `autopush enable` 發現自己的時段和別台重複時當場讓位,不必等到隔天排程執行。
  先前讓位只在排程跑的時候處理,所以兩台先各自 enable 再同步,還是會整天撞在一起。

## 1.0.54 - 2026-09-15

- 排程時間表改成一台機器一個檔。先前共用一份檔案,兩台各自寫整份再同步,git 得挑
  一個贏家,實測結果是兩邊各自只剩自己那筆,等於沒有協調到。分開之後 git 合併不需要
  做任何取捨。

## 1.0.53 - 2026-09-15

- 上次推送的時間改存在本機(`~/.local/state/acg/`),不再放進會同步的記憶目錄。
  那是每台各自的紀錄,同步之後一台的時間會變成另一台的,冷卻判斷就會跳過一台
  其實還沒推過的機器。

## 1.0.52 - 2026-09-15

- Desktop 的記憶頁可以看到與修改這台的自動上傳時間,也列出其他機器各自排在幾點。
  先前只有開關,時間只能用 CLI 設。

## 1.0.51 - 2026-09-15

- 各機器的自動推送時間改由共用時間表協調,不再全部擠在同一個小時。表放在記憶目錄的
  `autopush-schedule.toml`,`enable` 時自己認領一個空的分鐘,預設間隔十分鐘。實測
  兩台的隨機延遲會把它們挪到同一分鐘,等於沒錯開。
- 時間表的改動會自己生效。在一台改了別台的時段,那台下次排程執行時發現不一致就自己
  重建排程,不需要每台重跑 enable。兩台同時 enable 挑到同一分鐘時,名字排後面的
  自己讓位,兩邊各自計算會得到同樣結論,不需要溝通。
- 沒有記憶變更的機器也會接上遠端。時間表就住在記憶目錄裡,一台永遠不接上的機器會
  一直讀到自己那份舊的,看不到別台認領了什麼。
- Google Drive 模式下不再做落後判斷。那種儲存庫沒有上游參考,推送本來就是整份覆蓋。

## 1.0.50 - 2026-09-14

- 排程的單次執行加上 15 分鐘上限。先前 Windows 用的是工作排程器預設的 72 小時,
  一個卡住的工作會佔到後天;Linux 與 macOS 原本完全沒有上限。保存記憶是幾秒的事,
  超過十五分鐘就是卡住了,砍掉比藏著好。Windows 的上限在建立排程後另外設定,
  因為 `schtasks /Create` 沒有表達這件事的參數。

## 1.0.49 - 2026-09-14

- 修好排程在 Windows 卡住不退出的真正原因:獨立執行檔被工作排程器直接啟動時,
  行程數量剛好和雙擊一樣,acg 誤判成雙擊而停在「按 Enter 關閉視窗」。排程給的
  stdin 讀不到東西也不會拋 EOF,所以 `input()` 會一直等下去。現在沒有可互動的
  stdin 就不等待。上一版以為是 git 在等帳密,方向錯了,那時 git 根本還沒被啟動。
- Windows 的排程改成透過 `cmd /c` 啟動,不直接執行 exe。工作排程器給直接啟動的
  行程一組主控台程式用不了的標準控制代碼;隔一層 cmd 就正常。實測直接啟動會掛住,
  同一條命令列包一層 1.5 秒完成。

## 1.0.48 - 2026-09-14

- 排程推送不再卡住:git 執行時設 `GIT_TERMINAL_PROMPT=0` 並加上逾時。先前沒有
  終端機時 git 會停下來等帳密,Windows 實測行程活了 12 分鐘只燒 0.7 秒 CPU,
  其餘都在等 stdin。現在會明確失敗而不是靜止。
- 落後遠端的判斷改成先 fetch 再比。先前比的是本機快取的遠端狀態,兩天沒 pull
  的機器會顯示「一致」然後在推送時才撞上拒絕。
- 落後時自動 rebase 接上再推,多台機器在同一時間醒來不會互相拒絕。真的衝突則
  中止並留給人處理,無人看管時解衝突可能默默丟掉別人的筆記。

## 1.0.47 - 2026-09-14

- 自動推送在資料庫落後遠端時改為跳過並說明,不再回非零。落後是常態,排程每晚
  失敗一次只會讓人學會忽略它。要合併仍然要自己 `acg pull`,自動推送不代為處理。
- 發佈流程的測試矩陣補上 macOS,先前只有 `python-cli.yml` 那份加到。
...
- `memory status` 會指出索引漂移:有筆記沒被索引連到(下次開會話看不到它),
  或索引的連結指向不存在的檔案。索引的摘要是人寫的,無法自動重建,所以只回報
  不自動修。
- Desktop 的記憶頁一併顯示上面兩項:疑似含有憑證的筆記以錯誤樣式列出,索引漂移
  以警告列出,乾淨時整區隱藏。交接維持 CLI 專用,GUI 沒有 session 身分,認領者會是空的。
- `memory status` 會在筆記像是含有憑證時就說,不必等到 push 被擋下。掃描範圍
  只含會同步出去的檔案,本機日誌被 `memory/.gitignore` 排除,不在此列。憑證樣式
  改由 `safety` 模組統一定義,push 與 status 共用同一份。
...
- `push` 在暫存後沒有實質差異時回 0,不再回 1。換行符正規化這類變動會讓 git status
  看得到修改,但暫存差異是空的,先前會被當成失敗;現在那是 no-op。真正的錯誤仍回 1。
- `reset` 沒拿到確認而取消時回非 0,和其他指令一致。先前回 0,腳本會誤以為重設成功了。

## 1.0.43 - 2026-09-12

- 新增全域 `--force` / `-f`:所有 [y/N] 確認都當成同意,沒有終端機的 agent 和腳本
  因此走得完 pull → apply → push 全程。`reset` 例外,它會刪光設定檔,仍然要人回答。
- `push` 沒拿到確認而取消時改回非 0 的離開碼。先前回 0,腳本分不出「推上去了」和
  「什麼都沒推」,會誤判成功。
- Windows 上專案的 `.remember` 停在「已搬移」通知不再需要手動處理:外掛在 Git Bash
  底下寫的目的地是 `/c/Users/...` 這種 MSYS 寫法,Windows 打不開,正規化後也永遠
  不等於原生路徑,於是 hook 判定通知不符目的地而拒絕建立入口。現在比對前會把這種
  寫法翻成 `c:\Users\...`,資料本來就搬好了,只差那張通知沒換成連結。
...
- Desktop 的下載或上傳被私有儲存庫拒絕時,通知列直接出現「前往設定登入」按鈕。
- Desktop 設定裡多一顆「用 GitHub CLI 登入」:沒有內建瀏覽器登入的版本,按下去會
  開一個終端機視窗直接跑 gh 的登入,完成後按「重新檢查」就能選帳號;不再只有文字
  說明。
- 「改用 <帳號>」失敗時把 git 自己的錯誤帶出來,不再只說「無法確認能不能推送」。
- 狀態頁保留上次檢查結果並顯示「剛剛」「3 分鐘前」等時間;執行下載、套用或改動
  技能後不再整個清成「需重新檢查」,而是在原結果旁註明剛做了什麼、建議重新檢查。
