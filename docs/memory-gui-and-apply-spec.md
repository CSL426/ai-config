...
權杖才返回；執行中不返回，也不打斷操作。返回主要頁面時將焦點
交還入口按鈕；輸入法組字與長按 Esc 不應連續跳過畫面。

捲軸與下拉選單沿用暖色設計。下拉選單支援方向鍵、Home／End、
文字搜尋、Enter／Space 選取、Tab 離開與 Esc 關閉；停用狀態、
動態選項與原生 select 值同步。選單須在窄視窗內完整可操作。
